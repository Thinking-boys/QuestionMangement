...
$(function () {
    layui.use('table', function () {
        ...
        tableIns = table.render({
            ...
            , cols: [[
                {type: 'numbers'}
                , {field: 'id', title: '用户ID', width: 80, unresize: true, sort: true}
                , {field: 'username', title: '用户名称'}
                , {field: 'questionCount', title: '提问数量'}
                , {field: 'commentCount', title: '评论数量'}
                , {field: 'status', title: '是否禁用', width: 95, align: 'center', templet: '#isDelete'}
...
//     var version = nameVersion.substring(nameVersion.indexOf("_") + 1);
    ...
    var userIsDelete = checked ? "禁用" : "可用";
    //是否离职
    layer.confirm('您确定要把问题设置为' + userIsDelete + '状态吗？', {
        btn: ['确认', '返回'] //按钮
    }, function () {
        $.post("/wendaUser/setDeleteUser", {"id": id, "isDelete": isDelete}, function (data) {
            if (isLogin(data)) {
                if (data == "ok") {
                    //回调弹框
