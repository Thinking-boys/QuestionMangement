...
public class UserServiceImpl implements UserService {
	...
	public PageDataResult getUsers(UserSearchDTO userSearch, int page, int limit) {
		...
		if (null != userSearch) {
			if (StringUtils.isNotEmpty(userSearch.getInsertTimeStart())
				...
				userSearch.setInsertTimeEnd( DateUtil.dateTime2Str(new Date(),null));
			} else if (StringUtils.isEmpty(userSearch.getInsertTimeStart())
					&& StringUtils.isNotEmpty(userSearch.getInsertTimeEnd())) {
				userSearch.setInsertTimeStart(DateUtil.dateTime2Str(new Date(Integer.MIN_VALUE),null));
			}
			if (StringUtils.isNotEmpty(userSearch.getInsertTimeStart())
					&& StringUtils.isNotEmpty(userSearch.getInsertTimeEnd())) {
	...
	@Override
	public PageDataResult getWenDaUsers(WendaUserSearchDTO wendaUserSearchDTO, int page, int limit) {
		PageDataResult pdr = new PageDataResult();
		PageHelper.startPage(page, limit);
		List<UserResultDTO> users = wenDaUserDao.getUsers( wendaUserSearchDTO );
		// 获取分页查询后的数据
		PageInfo<UserResultDTO> pageInfo = new PageInfo<>(users);
		// 设置获取到的总记录数total：
		pdr.setTotals(Long.valueOf(pageInfo.getTotal()).intValue());
		if (null != users && users.size() > 0) {
			for (UserResultDTO ur : users) {
				Integer questionCount = questionDao.getQuestionCountByUserId( (int) ur.getId());
				Integer commentCount = commentDao.getCommentCountByUserId( (int)ur.getId());
				ur.setQuestionCount( questionCount == null ? 0 :questionCount );
				ur.setCommentCount( commentCount==null?0:commentCount);
			}
		}
		pdr.setList(users);
		return pdr;
	...
	public boolean updateDeleteStatus(int status, int id) {
		return wenDaUserDao.updateDeleteStatus( status,id )>0?true:false;
	}
}
